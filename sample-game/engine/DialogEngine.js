...
export class DialogEngine {
  constructor() {
    ...
    this.onBgChange = null; // callback for 'bg-change' nodes
  ...
  async processNode(node) {
    switch (node.type) {
      ...
      case 'bg-change':
        if (this.onBgChange) {
          this.onBgChange(node.image);
        }
        return;

      case 'bg-change-echo': {
        const image = this.choiceEcho ? this.choiceEcho.getEcho(node.echoId, node.variants, node.default) : node.default;
        if (this.onBgChange && image) {
          this.onBgChange(image);
        }
        return;
      }
